...
int main(int argc, char* argv[])
{
	...
	if (options.windowType.value < OPTVAL_UQM_WINDOW && isAddonAvailable(WINDOW_MODE(resolutionFactor, options.windowType.value)))
	{
			...
			case FUELRANGE_OPT:
				{
					int temp;
					if (parseIntOption(
							optarg, &temp, "Fuel range indicator")
						== -1)
					{
						badArg = true;
						break;
...
usage(FILE* out, const struct options_struct* defaults)
{
	...
	log_add(log_User, "  --menu      : menu type, pc=text, 3do=graphical "
					  "(default: %s)",
			choiceOptString(&defaults->whichMenu));
	...
	log_add(log_User, "  --showvisitstars : Dim visited stars on the "
					  ...
					  "(default: %s)",
			boolOptString(&defaults->showVisitedStars));
