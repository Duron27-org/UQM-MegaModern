				  ...
				  uio_MountHandle* relative)
{
	...
	if (uio_getPathPhysicalDirs(sourceDir, "", 0,
								&pDirHandles, &numPDirHandles, &treeItems)
		== -1)
	{
		// errno is set
		return NULL;
...
int uio_mkdir(uio_DirHandle* dir, const char* path, mode_t mode)
{
...
static inline void uio_EntriesContext_free(uio_EntriesContext* entriesContext);
