...
int zip_stat(uio_PDirHandle* pDirHandle, const char* name, struct stat* statBuf)
{
	uio_GPDirEntry* entry;
	...
	if (uio_GPDirEntry_isDir(entry) && entry->extra == nullptr)
	{
		...
		memset(statBuf, '\0', sizeof(struct stat));
		statBuf->st_mode = S_IFDIR | S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXOTH | S_IROTH | S_IWOTH | S_IXOTH;
		statBuf->st_uid = 0;
		statBuf->st_gid = 0;
		return 0;
	}
										...
										uio_FileBlock* fileBlock, off_t* pos)
{
	...
	*pos += 46;
	nextEntryOffset = *pos + fileNameLength + extraFieldLength + fileCommentLength;
		...
		case zip_OSType_VFAT:
			{
				...
				if (modeBytes == 0)
				{
					...
					return S_IFREG | S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;
				}
				if (modeBytes & 0x10)
				{
					// Directory
									  ...
									  uio_FileBlock* fileBlock, off_t* pos)
{
	...
	*pos += 26;
	nextEntryOffset = *pos + fileNameLength + extraFieldLength + gPFileData->compressedSize;
	if (isBitSet(gPFileData->compressionFlags, 3))
	{
									  ...
									  zip_GPFileData* gPFileData)
{
	...
	if (zip_updatePFileDataFromLocalFileHeader(gPFileData,
											   fileBlock, gPFileData->headerOffset)
		== -1)
	{
		int savedErrno = errno;
		uio_closeFileBlock(fileBlock);
