...
// freeing the original
void uio_GPDir_addFile(uio_GPDir* gPDir, const char* fileName, uio_GPFile* file)
{
						  ...
						  char* buf, size_t len)
{
	...
	while (!uio_GPDirEntries_iteratorDone(*iterator))
	{
		...
		if ((size_t)(sizeof(char*) + nameLen) > (size_t)(end - (char*)start))
		{
...
void uio_GPDir_closeEntries(uio_GPDirEntries_Iterator* iterator)
{
...
void uio_GPRoot_delete(void* arg)
{
...
void uio_GPFile_print(FILE* outStream, uio_GPFile* gPFile)
{
	(void)outStream;
	(void)gPFile;
}
