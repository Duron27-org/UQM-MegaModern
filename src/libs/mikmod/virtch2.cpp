...
static SLONG Mix32MonoNormal(const SWORD* const srce, SLONG* dest, SLONG idx, SLONG increment, SLONG todo)
{
	...
	while (todo--)
	{
		...
		sample = (SWORD)((((SLONG)(srce[i] * (FRACMASK + 1L - f)) + ((SLONG)srce[i + 1] * f)) >> FRACBITS));
		idx += increment;

		if (vnf->rampvol)
		{
			*dest++ += (long)((((SLONG)(vnf->oldlvol * vnf->rampvol) + (vnf->lvolsel * (CLICK_BUFFER - vnf->rampvol))) * (SLONG)sample) >> CLICK_SHIFT);
			vnf->rampvol--;
		}
		else if (vnf->click)
		{
			*dest++ += (long)(((((SLONG)vnf->lvolsel * (CLICK_BUFFER - vnf->click)) * (SLONG)sample) + (vnf->lastvalL * vnf->click)) >> CLICK_SHIFT);
			vnf->click--;
		}
		else
		{
...
static SLONG Mix32StereoNormal(const SWORD* const srce, SLONG* dest, SLONG idx, SLONG increment, ULONG todo)
{
	...
	while (todo--)
	{
		...
		{
			*dest++ += (long)(((((SLONG)vnf->oldlvol * vnf->rampvol) + (vnf->lvolsel * (CLICK_BUFFER - vnf->rampvol))) * (SLONG)sample) >> CLICK_SHIFT);
			*dest++ += (long)(((((SLONG)vnf->oldrvol * vnf->rampvol) + (vnf->rvolsel * (CLICK_BUFFER - vnf->rampvol))) * (SLONG)sample) >> CLICK_SHIFT);
			vnf->rampvol--;
		}
		else if (vnf->click)
		{
			*dest++ += (long)((((SLONG)(vnf->lvolsel * (CLICK_BUFFER - vnf->click)) * (SLONG)sample) + (vnf->lastvalL * vnf->click))
							  >> CLICK_SHIFT);
			*dest++ += (long)(((((SLONG)vnf->rvolsel * (CLICK_BUFFER - vnf->click)) * (SLONG)sample) + (vnf->lastvalR * vnf->click))
							  >> CLICK_SHIFT);
			vnf->click--;
		}
		else
...
static SLONG Mix32StereoSurround(const SWORD* const srce, SLONG* dest, SLONG idx, SLONG increment, ULONG todo)
{
	...
	while (todo--)
	{
		...
		if (vnf->rampvol)
		{
			whoop = (long)((((SLONG)(vnf->oldlvol * vnf->rampvol) + (vnf->lvolsel * (CLICK_BUFFER - vnf->rampvol))) * (SLONG)sample) >> CLICK_SHIFT);
			*dest++ += whoop;
			*dest++ -= whoop;
			vnf->rampvol--;
		}
		...
		{
			whoop = (long)(((((SLONG)vnf->lvolsel * (CLICK_BUFFER - vnf->click)) * (SLONG)sample) + (vnf->lastvalL * vnf->click)) >> CLICK_SHIFT);
			*dest++ += whoop;
			*dest++ -= whoop;
			vnf->click--;
		}
...
static SLONGLONG MixMonoNormal(const SWORD* const srce, SLONG* dest, SLONGLONG idx, SLONGLONG increment, SLONG todo)
{
	...
	{
		i = idx >> FRACBITS, f = idx & FRACMASK;
		sample = (SWORD)((((SLONGLONG)(srce[i] * (FRACMASK + 1L - f)) + ((SLONGLONG)srce[i + 1] * f)) >> FRACBITS));
		idx += increment;

		if (vnf->rampvol)
		{
			*dest++ += (long)((((SLONGLONG)(vnf->oldlvol * vnf->rampvol) + (vnf->lvolsel * (CLICK_BUFFER - vnf->rampvol))) * (SLONGLONG)sample) >> CLICK_SHIFT);
			vnf->rampvol--;
		}
		else if (vnf->click)
		{
			*dest++ += (long)(((((SLONGLONG)vnf->lvolsel * (CLICK_BUFFER - vnf->click)) * (SLONGLONG)sample) + (vnf->lastvalL * vnf->click)) >> CLICK_SHIFT);
			vnf->click--;
		}
		else
		{
...
static __inline SWORD GetSample(const SWORD* const srce, SLONGLONG idx)
{
	...
	return (SWORD)(((((SLONGLONG)srce[i] * (FRACMASK + 1L - f)) + ((SLONGLONG)srce[i + 1] * f)) >> FRACBITS));
}
...
static SLONGLONG MixSIMDStereoNormal(const SWORD* const srce, SLONG* dest, SLONGLONG idx, SLONGLONG increment, ULONG todo)
{
	...
	remain = todo & 3;
	{
		...
		for (todo >>= 2; todo; todo--)
		{
			...
			r1 = vec_ld(0, s);
			v1 = vec_perm(r1, r1, (vector unsigned char)(0 * 2, 0 * 2 + 1, /* s0 */
														 4 * 2, 4 * 2 + 1, /* 0  */
														 0 * 2, 0 * 2 + 1, /* s0 */
														 4 * 2, 4 * 2 + 1, /* 0  */
														 1 * 2, 1 * 2 + 1, /* s1 */
			...
			v2 = vec_perm(r1, r1, (vector unsigned char)(2 * 2, 2 * 2 + 1, /* s2 */
														 4 * 2, 4 * 2 + 1, /* 0  */
														 2 * 2, 2 * 2 + 1, /* s2 */
														 4 * 2, 4 * 2 + 1, /* 0  */
														 3 * 2, 3 * 2 + 1, /* s3 */
...
static SLONGLONG MixStereoNormal(const SWORD* const srce, SLONG* dest, SLONGLONG idx, SLONGLONG increment, ULONG todo)
{
	...
	if (vnf->rampvol)
	{
		while (todo)
		{
			...
			*dest++ += (long)(((((SLONGLONG)vnf->oldlvol * vnf->rampvol) + (vnf->lvolsel * (CLICK_BUFFER - vnf->rampvol))) * (SLONGLONG)sample) >> CLICK_SHIFT);
			*dest++ += (long)(((((SLONGLONG)vnf->oldrvol * vnf->rampvol) + (vnf->rvolsel * (CLICK_BUFFER - vnf->rampvol))) * (SLONGLONG)sample) >> CLICK_SHIFT);
			vnf->rampvol--;
	...
	if (vnf->click)
	{
		while (todo)
		{
			...
			*dest++ += (long)((((SLONGLONG)(vnf->lvolsel * (CLICK_BUFFER - vnf->click)) * (SLONGLONG)sample) + (vnf->lastvalL * vnf->click))
							  >> CLICK_SHIFT);

			*dest++ += (long)(((((SLONGLONG)vnf->rvolsel * (CLICK_BUFFER - vnf->click)) * (SLONGLONG)sample) + (vnf->lastvalR * vnf->click))
							  >> CLICK_SHIFT);
			vnf->click--;
...
static SLONGLONG MixStereoSurround(const SWORD* srce, SLONG* dest, SLONGLONG idx, SLONGLONG increment, ULONG todo)
{
	...
	while (todo--)
	{
		...
		if (vnf->rampvol)
		{
			whoop = (long)((((SLONGLONG)(vnf->oldlvol * vnf->rampvol) + (vnf->lvolsel * (CLICK_BUFFER - vnf->rampvol))) * (SLONGLONG)sample) >> CLICK_SHIFT);
			*dest++ += whoop;
			*dest++ -= whoop;
			vnf->rampvol--;
		}
		...
		{
			whoop = (long)(((((SLONGLONG)vnf->lvolsel * (CLICK_BUFFER - vnf->click)) * (SLONGLONG)sample) + (vnf->lastvalL * vnf->click)) >> CLICK_SHIFT);
			*dest++ += whoop;
			*dest++ -= whoop;
			vnf->click--;
		}
...
static void MixReverb_Normal(SLONG* srce, NATIVE count)
{
	...
	while (count--)
	{
		...
		/* left channel */
		*srce++ += RVbufL1[loc1] - RVbufL2[loc2] + RVbufL3[loc3] - RVbufL4[loc4] + RVbufL5[loc5] - RVbufL6[loc6] + RVbufL7[loc7] - RVbufL8[loc8];
	}
}
...
static void MixReverb_Stereo(SLONG* srce, NATIVE count)
{
	...
	while (count--)
	{
		...
		*srce++ += RVbufL1[loc1] - RVbufL2[loc2] + RVbufL3[loc3] - RVbufL4[loc4] + RVbufL5[loc5] - RVbufL6[loc6] + RVbufL7[loc7] - RVbufL8[loc8];
		...
		/* right channel */
		*srce++ += RVbufR1[loc1] - RVbufR2[loc2] + RVbufR3[loc3] - RVbufR4[loc4] + RVbufR5[loc5] - RVbufR6[loc6] + RVbufR7[loc7] - RVbufR8[loc8];
	}
}
...
#define CHECK_SAMPLE_FP(var, bound) var = (var > bound) ? bound : (var < -bound) ? -bound : \
																				   var
...
static void Mix32ToFP_Normal(float* dste, const SLONG* srce, NATIVE count)
{
	...
	for (count /= SAMPLING_FACTOR; count; count--)
	{
		...
		{
			EXTRACT_SAMPLE_FP(x1, 1.0f);
			EXTRACT_SAMPLE_FP(x2, 1.0f);
...
static void Mix32ToFP_Stereo(float* dste, const SLONG* srce, NATIVE count)
{
	...
	for (count /= SAMPLING_FACTOR; count; count--)
	{
		...
		for (i = SAMPLING_FACTOR / 2; i; i--)
		{
			EXTRACT_SAMPLE_FP(x1, 1.0f);
			EXTRACT_SAMPLE_FP(x2, 1.0f);
			EXTRACT_SAMPLE_FP(x3, 1.0f);
			EXTRACT_SAMPLE_FP(x4, 1.0f);
...
#define CHECK_SAMPLE(var, bound) var = (var >= bound) ? bound - 1 : (var < -bound) ? -bound : \
																					 var
...
static void Mix32To16_Stereo(SWORD* dste, const SLONG* srce, NATIVE count)
{
	...
	for (count /= SAMPLING_FACTOR; count; count--)
	{
		...
		for (i = SAMPLING_FACTOR / 2; i; i--)
		{
			...
			CHECK_SAMPLE(x1, 32768);
			CHECK_SAMPLE(x2, 32768);
			CHECK_SAMPLE(x3, 32768);
			CHECK_SAMPLE(x4, 32768);
...
static void Mix32To8_Normal(SBYTE* dste, const SLONG* srce, NATIVE count)
{
	...
	for (count /= SAMPLING_FACTOR; count; count--)
	{
		...
		for (i = SAMPLING_FACTOR / 2; i; i--)
		{
			EXTRACT_SAMPLE(x1, 256);
			EXTRACT_SAMPLE(x2, 256);
...
static void Mix32To8_Stereo(SBYTE* dste, const SLONG* srce, NATIVE count)
{
	...
	for (count /= SAMPLING_FACTOR; count; count--)
	{
		...
		for (i = SAMPLING_FACTOR / 2; i; i--)
		{
			EXTRACT_SAMPLE(x1, 256);
			EXTRACT_SAMPLE(x2, 256);
			EXTRACT_SAMPLE(x3, 256);
			EXTRACT_SAMPLE(x4, 256);
...
static void Mix32To16_Stereo_SIMD_4Tap(SWORD* dste, const SLONG* srce, NATIVE count)
{
	...
	for (count >>= 4; count; count--)
	{
		...
		__m128i v0 = _mm_add_epi32(
			...
			_mm_srai_epi32(_mm_loadu_si128((__m128i const*)(srce + 4)), SHIFT_MIX_TO_16)); /* v0: s0.l+s2.l | s0.r+s2.r | s1.l+s3.l | s1.r+s3.r */
		...
		__m128i v2 = _mm_add_epi32(
			...
			_mm_srai_epi32(_mm_loadu_si128((__m128i const*)(srce + 12)), SHIFT_MIX_TO_16)); /* v2: s4.l+s6.l | s4.r+s6.r | s5.l+s7.l | s5.r+s7.r */
		...
		v0 = _mm_add_epi32(
			...
			_mm_srai_epi32(_mm_loadu_si128((__m128i const*)(srce + 20)), SHIFT_MIX_TO_16)); /* 128bit = 2 stereo samples */
...
static void Mix32To16_Stereo_SIMD_4Tap(SWORD* dste, const SLONG* srce, NATIVE count)
{
	...
	for (count >>= 4; count; count--)
	{
		...
		vector signed int v0 = vec_add(
			...
			vec_sra(vec_ld(0x10, srce), SHIFT_MIX_TO_16)); /* 128bit = 2 stereo samples */
		...
		v0 = vec_add(
			...
			vec_sra(vec_ld(0x50, srce), SHIFT_MIX_TO_16)); /* 128bit = 2 stereo samples */
		...
		if (vnf->vol || vnf->rampvol)
		{
			...
			if ((vnf->current < 0x7fffffff) && (endpos < 0x7fffffff))
			{
				if (vc_mode & DMODE_STEREO)
				{
					...
					{
						vnf->current = (SLONGLONG)Mix32StereoSurround(s, ptr, vnf->current, vnf->increment, done);
					}
					else
					{
