...
static void vd_computeMasks(uint32_t mask, uint32_t* shift, uint32_t* loss);
...
VideoDecoder_Load(uio_DirHandle* dir, const char* filename)
{
	...
	for (info = vd_decoders; info->used && (!info->ext || strcmp(info->ext, pext) != 0);
		 ++info)
		;
	if (!info->ext)
	{
...
// BEGIN: adapted from SDL
static void
vd_computeMasks(uint32_t mask, uint32_t* shift, uint32_t* loss)
{
	*shift = 0;
	*loss = 8;
	if (mask)
