...
static uint32_t dukv_GetStructSize(void);
...
static uint32_t dukv_SeekFrame(THIS_PTR, uint32_t frame);
static float dukv_SeekTime(THIS_PTR, float time);
static uint32_t dukv_GetFrame(THIS_PTR);
...
{
	uint32_t version;
	uint32_t scrn_x_ofs; // horz screen offset in pixels
	uint32_t scrn_y_ofs; // vert screen offset in pixels
	uint16_t wb, hb;	 // width + height in blocks
	int16_t lumas[8];	 // future luminance deltas
	int16_t chromas[8];	 // future chrominance deltas
...
{
	int32_t lumas[NUM_VECTORS][NUM_VEC_ITEMS];
	int32_t chromas[NUM_VECTORS][NUM_VEC_ITEMS];
...
typedef struct tfb_duckvideodecoder
{
	...
	uint32_t iframe;
	uint32_t version;
	uint32_t wb, hb; // width, height in blocks
...
static void
dukv_DecodeFrame(uint8_t* src_p, uint32_t* dst_p, uint32_t wb, uint32_t hb,
				 TFB_DuckVideoDeltas* deltas)
{
	...
	for (y = 0; y < hb; ++y)
	{
		int32_t accum0, accum1, corr, corr0, corr1, delta;
		int32_t pix[4];
...
static void
dukv_DecodeFrameV3(uint8_t* src_p, uint32_t* dst_p, uint32_t wb, uint32_t hb,
				   TFB_DuckVideoDeltas* deltas)
{
	int iVec;
	int iSeq;
...
dukv_ReadFrames(TFB_DuckVideoDecoder* dukv)
{
	...
	dukv->cframes = uio_ftell(fp) / sizeof(uint32_t);
	uio_fseek(fp, 0, SEEK_SET);
	dukv->frames = (uint32_t*)HMalloc(dukv->cframes * sizeof(uint32_t));

	if (uio_fread(dukv->frames, sizeof(uint32_t), dukv->cframes,
				  fp)
		!= dukv->cframes)
	{
		HFree(dukv->frames);
...
static bool
dukv_ReadVectors(TFB_DuckVideoDecoder* dukv, uint8_t* vectors)
{
	uio_Stream* fp;
	char filename[280] {};
	int ret;
...
static bool
dukv_ReadHeader(TFB_DuckVideoDecoder* dukv, int32_t* pl, int32_t* pc)
{
	uio_Stream* fp;
	char filename[280] {};
	int ret;
	...
	for (i = 0; i < 8; ++i)
	{
		pl[i] = (int16_t)UQM_SwapBE16(hdr.lumas[i]);
		pc[i] = (int16_t)UQM_SwapBE16(hdr.chromas[i]);
	}
...
static int32_t
dukv_make_delta(int32_t* protos, bool is_chroma, int i1, int i2)
{
	int32_t d1, d2;
...
static int32_t
dukv_make_corr(int32_t* protos, bool is_chroma, int i1, int i2)
{
	int32_t d1, d2;
...
static void
dukv_DecodeVector(uint8_t* vec, int32_t* p, bool is_chroma, int32_t* deltas)
{
	int citems = vec[0];
	int i;
	...
	{
		int32_t d = dukv_make_delta(p, is_chroma, vec[1], vec[2]);
...
static void
dukv_InitDeltas(TFB_DuckVideoDecoder* dukv, uint8_t* vectors,
				int32_t* pl, int32_t* pc)
{
	int i;

	for (i = 0; i < NUM_VECTORS; ++i)
	{
		uint8_t* vector = vectors + i * NUM_VEC_ITEMS;
...
dukv_PixelConv(uint16_t pix, const TFB_PixelFormat* fmt)
{
	uint32_t r, g, b;
	...
	return ((r >> fmt->Rloss) << fmt->Rshift) | ((g >> fmt->Gloss) << fmt->Gshift) | ((b >> fmt->Bloss) << fmt->Bshift);
}
	...
	switch (fmt->BytesPerPixel)
	{
		case 2:
			for (y = 0; y < h; ++y)
			{
				...
				dst0 = (uint16_t*)This->callbacks.GetCanvasLine(
					This, y * 2 + IF_HD(1));
				dst1 = (uint16_t*)This->callbacks.GetCanvasLine(
					This, y * 2 + 1);
				...
				for (x = 0; x < dukv->decoder.w; ++x, chooseIfHd(++dec, reinterpret_cast<uint32_t*>(++bufInc)), ++dst0, ++dst1)
				{
					/*if (IS_HD)
						++bufInc;
					else
					...
					pair = *dec;
					*dst0 = dukv_PixelConv((uint16_t)(pair >> 16), fmt);
					*dst1 = dukv_PixelConv((uint16_t)(pair & 0xffff), fmt);
				}
			}
			break;
		case 3:
			for (y = 0; y < h; ++y)
			{
				...
				dst0 = (uint8_t*)This->callbacks.GetCanvasLine(
					This, y * 2 + IF_HD(1));
				dst1 = (uint8_t*)This->callbacks.GetCanvasLine(
					This, y * 2 + 1);
				...
				for (x = 0; x < dukv->decoder.w; ++x, chooseIfHd(++dec, reinterpret_cast<uint32_t*>(++bufInc)), dst0 += 3, dst1 += 3)
				{
					/*if (IS_HD)
						++bufInc;
					else
					...
					pair = *dec;
					*(uint32_t*)dst0 =
						dukv_PixelConv((uint16_t)(pair >> 16), fmt);
					*(uint32_t*)dst1 =
						dukv_PixelConv((uint16_t)(pair & 0xffff), fmt);
				}
			}
			break;
		case 4:
			for (y = 0; y < h; ++y)
			{
				...
				dst0 = (uint32_t*)This->callbacks.GetCanvasLine(
					This, y * 2);
				dst1 = (uint32_t*)This->callbacks.GetCanvasLine(
					This, y * 2 + 1);
...
dukv_Open(THIS_PTR, uio_DirHandle* dir, const char* filename)
{
	...
	vectors = (uint8_t*)HMalloc(NUM_VEC_ITEMS * NUM_VECTORS);
	...
	This->interframe_wait = (uint32_t)(1000.0 / DUCK_GENERAL_FPS);

	dukv->inbuf = (uint8_t*)HMalloc(DUCK_MAX_FRAME_SIZE);
	dukv->decbuf = (uint32_t*)HMalloc(
		dukv->decoder.w * dukv->decoder.h * sizeof(uint16_t));
...
dukv_DecodeNext(THIS_PTR)
{
	...
	ver = UQM_SwapBE16(*(uint16_t*)dukv->inbuf);
	if (ver == 0x0300)
	{
	...
	{
		This->callbacks.SetTimer(This, (uint32_t)(1000.0f / DUCK_GENERAL_FPS));
	}
...
static uint32_t
dukv_SeekFrame(THIS_PTR, uint32_t frame)
{
	TFB_DuckVideoDecoder* dukv = (TFB_DuckVideoDecoder*)This;
...
{
	//TFB_DuckVideoDecoder* dukv = (TFB_DuckVideoDecoder*) This;
	uint32_t frame = (uint32_t)(time * DUCK_GENERAL_FPS);
