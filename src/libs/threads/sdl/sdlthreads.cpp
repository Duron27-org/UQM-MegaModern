...
void PrintThreadsStats_SDL(void)
{
	TrueThread ptr;
	int now;
...
void DestroyThread_SDL(Thread t)
{
	HFree(t);
}

Thread
CreateThread_SDL(ThreadFunction func, void* data, int32_t stackSize
#ifdef NAMED_SYNCHRO
				 ,
				 const char* name
#endif
...
Mutex
#ifdef NAMED_SYNCHRO
CreateMutex_SDL(const char* name, uint32_t syncClass)
#else
...
Semaphore
CreateSemaphore_SDL(uint32_t initial
#ifdef NAMED_SYNCHRO
					,
					const char* name, uint32_t syncClass
#endif
)
{
...
void ClearSemaphore_SDL(Semaphore s)
{
	Sem* sem = (Sem*)s;
...
RecursiveMutex
#ifdef NAMED_SYNCHRO
CreateRecursiveMutex_SDL(const char* name, uint32_t syncClass)
#else
...
void DestroyRecursiveMutex_SDL(RecursiveMutex val)
{
	RecM* mtx = (RecM*)val;
...
void LockRecursiveMutex_SDL(RecursiveMutex val)
{
	RecM* mtx = (RecM*)val;
...
void UnlockRecursiveMutex_SDL(RecursiveMutex val)
{
	RecM* mtx = (RecM*)val;
	SDL_threadID thread_id = SDL_ThreadID();
	if (!mtx->locks || mtx->thread_id != thread_id)
...
int GetRecursiveMutexDepth_SDL(RecursiveMutex val)
{
	RecM* mtx = (RecM*)val;
	return mtx->locks;
}
...
CondVar
#ifdef NAMED_SYNCHRO
CreateCondVar_SDL(const char* name, uint32_t syncClass)
#else
